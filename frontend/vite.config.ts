...
const testsIncludeRoot = relative(process.cwd(), resolve(__dirname, 'src')).replace(/\\/g, '/');
...
export default defineConfig(({ command }) => {
    ...
    const base = command === 'build' ? process.env['WALDIEZ_STUDIO_BASE_URL'] || '/' : '/';
    const publicDir =
        command === 'build'
